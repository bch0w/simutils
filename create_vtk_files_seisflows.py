...
# Set user parameters
seisflows_dir = "chkbd_twoevent"
kernels = ["vs_kernel"]
models = ["vs"]

# Can set these paths if they change
basepath = ("/scale_wlg_nobackup/filesets/nobackup/nesi00263/bchow/tests/"
            "seisflows_test/hikurangi_trial")
specfem = os.path.join(basepath, "specfem_6895e2f7")
output_script = os.path.join(specfem, 'run_xcombine_vol_data_vtk.sh')

# Auto finish pathing
input_sum_dir = "INPUT_SUM"
output_sum_dir = f"OUTPUT_SUM/{seisflows_dir}"
seisflows = os.path.join(basepath, seisflows_dir)
...
glob_kernels = os.path.join(seisflows, "scratch", "evalgrad", "kernels", "*")
...
if method == "build":
    with open(output_script, 'w') as f:
        ...
        for kernel_dir in glob.glob(glob_kernels):
            # Get the name of the kernel
            kernel_fid = os.path.basename(kernel_dir)
            print(f"kernel {kernel_fid}")
            ...
            for kernel in kernels:
                ...
                for src in glob.glob(
                    ...
                    dst_fid = f"{proc_name}_{kernel_fid}.bin"
                    
                    dst = os.path.join(input_sum, dst_fid)
                    os.symlink(src, dst)

                # Write the srun call
                kernel_id = f"{kernel}_{kernel_fid}"
