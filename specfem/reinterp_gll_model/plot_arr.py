...
def plot_single(arr, choice, val):
    """"""
    ...
    x_ = np.unique(xyd[:, 0])
    y_ = np.unique(xyd[:, 1])
    z_ = np.reshape(xyd[:, 2], (len(y_), len(x_)))
    sc = plt.contourf(x_, y_, z_, levels=128, cmap=CMAP)

    plt.colorbar(sc, label="Vs [m/s]")


    # Finish off the plot
    plt.title(f"{CHOICE} ({choiceval}~={val*1e-3:.2f}km)")

    plt.savefig(f"{CHOICE}_{choiceval}_{int(val)}.png")
...
if __name__ == "__main__":
    ...
    if section == "shallow":
        ...
        # Read in data for both raw and smoothed models
        data = np.loadtxt(f"{section}_tomo_file.xyz")
        ...
        for xval in [arr[:,0].min(), arr[:,0].mean(), arr[:,0].max()]:
            plot_single(arr, choice=0, val=xval)

        for yval in [arr[:,1].min(), arr[:,1].mean(), arr[:,1].max()]:
            plot_single(arr, choice=1, val=yval)

        for zval in np.arange(2.25E3, -8E3, -250):
            plot_single(arr, choice=2, val=zval)
