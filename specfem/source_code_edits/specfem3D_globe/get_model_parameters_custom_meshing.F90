...
! with the planetary constants like R_PLANET, RHOAV, etc.
  ...
  !!! CUSTOM - Allow s40rts + crust1 to be used
  case ('custom_s40rts_crust1')
    ONE_CRUST = .false.  ! true = 1 crust layer; false = 2 crust layers
    CASE_3D = .false.   ! true = more GLL points / km in upper crust
    CRUSTAL = .false.   ! true: 3D crustal model; false: 1D crustal model
    MODEL_3D_MANTLE_PERTUBATIONS = .false.   ! true: perturb ref mantle model
  ...
  !!! CUSTOM - Make s40rts + crust1 compatible with GLL
  case ('custom_3d_gll')
    ONE_CRUST = .false.  ! default .true.
    CASE_3D = .false.    ! default .true.
    CRUSTAL = .false.    ! default .true.
...
!---
  ...
  if (HONOR_1D_SPHERICAL_MOHO) then
    ...
    case (IPLANET_EARTH)
      ...
      ! CHOW EDIT: forcing fictitious moho to 60km depth, pretty kludgey
      ! RMOHO_FICTITIOUS_IN_MESHER = (R120 + R_PLANET) / 2.0d0  ! 100km
      RMOHO_FICTITIOUS_IN_MESHER = (R80 + R_PLANET) / 2.0d0 - 60000.0d0
      print *,'RMOHO ', RMOHO_FICTITIOUS_IN_MESHER
    case (IPLANET_MARS)
      ! Mars
