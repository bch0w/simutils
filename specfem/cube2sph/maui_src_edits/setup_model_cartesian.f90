program setup_model_cartesian
  !! program to setup velocity model for a sphere-like mesh in SPECFEM3D
  !! Tianshi Liu, 2021.1
  !! 1. read in reference mesh (without topography, ellipticity, Moho, etc.) 
  !!    file external_mesh.bin
  !! 2. read in default model rho.bin, vp.bin, vs.bin
  !! 3. set reference model using subroutine update_parameters_ref, 
  !!    1D model -> 3D mantle model -> crustal model
  !! 4. (optional) set netcdf model using subroutine
  !!    update_parameters_from_netcdf
  !! 5. write new velocity files rho.bin, vp.bin, vs.bin ready to be used
  !!    by generate_databases
  use mpi
...
subroutine update_parameters_ref(x, y, z, rho_old, vp_old, vs_old, rho_new, &
  ...
  !! subroutine to assign reference model to a sphere-like mesh
  !! Tianshi Liu, 2021.1
  !! modified from SPECFEM3D_GLOBE
  !! 1. set parameters
  !! 2. if using crustal model: do MOHO stretching
  !! 3. set 1D model (PREM)
  !! 4. set 3D mantle model (S40RTS)
  !! 5. if using crustal model: set Crust1.0 model
  use constants
  ...
  !! MODIFY here to choose a minimum Vs to avoid a too small element size
  !! if a smaller Vs is assigned for some reason, the Vs value is reset 
  !! to vs_min, and the Vp value is scaled to maintain a fixed Poisson ratio
  vs_min = 1000.0  ! m/s
  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  !vs_min = 0.0
...
!  character(len = *), parameter :: model_fn = 'AK_EMB_2019JGRSE/MCMC_EMBerg_highres_2019_AK_resampled.nc'
!  double precision, parameter :: depth_start = 0.0, lat_start = 50.0, lon_start = -180.0, &
!                            ddepth = 1.0, dlat = 0.1, dlon = 0.2
!  integer, parameter :: ndepth = 145, nlat = 251, nlon = 301
!  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
