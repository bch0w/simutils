...
def read_xy(fid):
    ...
    data = np.loadtxt(fid)
    x, y = data.T
    x = x % 360  # avoid negative values for longitude
    return x, y
...
def plot(fid, x, y, 
         ...
         extent=(120, 195., 42.5, 77.5),  
         source=(129.0297, 41.3324), 
         station=None,  # (-153.5058, 66.5156), 
         dpi=300, 
         axis_lw=2, 
         min_val=-1e-5, 
         max_val=1e-5, 
...
if __name__ == "__main__":
    ...
    if make_pngs:
        ...
        if not files:
            files = find(input_path, file_ext)[:100]
