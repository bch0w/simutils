...
! determines the number of external tomographic models and sets a total maximum number of element records
  ...
  if (nundefMat_ext_mesh == 0 .and. IMODEL == IMODEL_TOMO) then
    !nmaterials = 1
    nmaterials = 3 ! UGLY HACK - MODIFY THIS LINE TO SUIT YOUR NEEDS
  endif
  ...
  do iundef = 1, nmaterials
    ...
    if (nundefMat_ext_mesh == 0 .and. IMODEL == IMODEL_TOMO) then
      ...
      ! UGLY HACK - MODIFY THESE LINES TO SUIT YOUR NEEDS
      if (iundef == 1) filename = 'tomography_model_mantle.xyz'
      if (iundef == 2) filename = 'tomography_model_crust.xyz'
      if (iundef == 3) filename = 'tomography_model_shallow.xyz'
...
!
  ...
  ! write(*,*) 'imaterial_id', imaterial_id ! DEBUGGING
