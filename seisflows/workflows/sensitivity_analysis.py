...
class SensitivityAnalysis(custom_import("workflow", "base")):
    """
    ...
    def check(self):
        ...
        # Local can be used for seismic data lookup  
        if "DATA" not in PATH:
    ...
    def main(self):
        ...
        # Used for when changing bandpass, no need to recalc syns 
        # print("\tEvaluating gradient m")
        # self.evaluate_gradient_no_forward(model=m, path=path1)
        ...
        # self.evaluate_gradient_no_forward(model=(m + dm), path=path2)
    ...
    def evaluate_gradient_no_forward(self, model, path):
        """ 
        Performs forward simulation to evaluate objective function and adjoint
        simulation to generate misfit kernels
        """
